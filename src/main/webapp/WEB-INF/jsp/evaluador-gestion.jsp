...
<%!
    ...
    Long idLong(String valor) {
        try { return valor == null || valor.isBlank() ? null : Long.valueOf(valor); }
        catch (NumberFormatException ex) { return null; }
    }
    GrupoEvaluacion grupoPorId(List<GrupoEvaluacion> grupos, Long id) {
        if (grupos == null || grupos.isEmpty()) return null;
        if (id != null) for (GrupoEvaluacion g : grupos) if (g.getId().equals(id)) return g;
        return grupos.get(0);
    }
    AplicacionPrueba ultimaAplicacion(Evaluado evaluado, List<AplicacionPrueba> aplicaciones) {
        AplicacionPrueba ultima = null;
        if (evaluado == null || aplicaciones == null) return null;
        for (AplicacionPrueba a : aplicaciones) {
            if (a.getEvaluado() == null || !evaluado.getId().equals(a.getEvaluado().getId())) continue;
            if (ultima == null || a.getId() > ultima.getId()) ultima = a;
        }
        return ultima;
    }
    String idsGrupos(Evaluado evaluado) {
        if (evaluado == null || evaluado.getGrupos() == null) return "";
        StringBuilder salida = new StringBuilder();
        for (GrupoEvaluacion g : evaluado.getGrupos()) {
            if (g.getId() != null) salida.append(g.getId()).append(' ');
        }
        return salida.toString();
    }
    String nombresGrupos(Evaluado evaluado) {
        if (evaluado == null || evaluado.getGrupos() == null) return "";
        StringBuilder salida = new StringBuilder();
...
<%
    ...
    Long grupoAsignacionId = idLong(request.getParameter("grupoId"));
    if (grupoAsignacionId == null && grupos != null && !grupos.isEmpty()) grupoAsignacionId = grupos.get(0).getId();
%>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        body { margin: 0; font-family: Arial, Helvetica, sans-serif; color: #1f2937; background: linear-gradient(180deg, #e6f1f8 0, #f4f8fb 260px, #eef3f7 100%); }
        ...
        nav a, .button { border: 0; border-radius: 6px; padding: 10px 13px; background: #dceaf2; color: #243441; font-weight: 700; text-decoration: none; cursor: pointer; }
        nav a.active, .button.primary { background: #1f6f8b; color: white; box-shadow: 0 8px 18px rgba(31, 111, 139, .18); }
        .top { display: flex; justify-content: flex-end; gap: 12px; align-items: center; margin-bottom: 16px; padding: 12px; background: linear-gradient(135deg, #ffffff, #e9f5fa); border: 1px solid #c5d8e2; border-radius: 8px; }
        ...
        .panel { background: linear-gradient(180deg, #ffffff, #f5fbfe); border: 1px solid #bfd6e4; border-radius: 8px; padding: 18px; box-shadow: 0 14px 30px rgba(31, 75, 93, .10); }
        .panel > h2:first-child { margin: -18px -18px 16px; padding: 14px 18px; color: white; background: linear-gradient(135deg, #16476a, #1f7fa3); border-radius: 8px 8px 0 0; }
        ...
        select {
            ...
            color: #15384f;
            font-weight: 800;
            border: 1px solid #7fb8ce;
            background:
                linear-gradient(45deg, transparent 50%, #1f6f8b 50%) calc(100% - 22px) 50% / 7px 7px no-repeat,
                linear-gradient(135deg, #ffffff 0%, #eef8fc 100%);
            box-shadow: inset 0 1px 0 rgba(255,255,255,.8), 0 8px 18px rgba(31, 111, 139, .08);
        }
        select:hover { border-color: #1f7fa3; background-color: #f4fbfe; }
        input:focus, select:focus, textarea:focus { outline: 3px solid rgba(31, 111, 139, .20); border-color: #1f6f8b; }
        ...
        .muted { color: #64748b; }
        .section-toolbar { display: flex; justify-content: space-between; gap: 14px; align-items: center; margin: -18px -18px 16px; padding: 18px; background: linear-gradient(135deg, #16476a, #1f7fa3); color: white; border-radius: 8px 8px 0 0; }
        .section-toolbar h2 { margin: 0 0 4px; }
        .section-toolbar p { margin: 0; }
        .section-toolbar h2, .section-toolbar .muted { color: white; }
        .search-box { min-width: 260px; max-width: 360px; position: relative; }
        .search-box input { background: rgba(255,255,255,.96); border-color: rgba(255,255,255,.55); min-height: 46px; }
        .result-filters { display: grid; grid-template-columns: repeat(4, minmax(150px, 1fr)); gap: 10px; margin-bottom: 16px; padding: 16px; border: 1px solid #9cc9d8; border-radius: 8px; background: linear-gradient(135deg, #dff1f8, #ffffff); box-shadow: 0 10px 22px rgba(31,111,139,.08); }
        .result-filters label { font-size: 12px; color: #20465f; text-transform: uppercase; letter-spacing: .02em; }
        .result-filters select { margin-top: 6px; border-color: #74b1c9; box-shadow: 0 6px 14px rgba(31, 111, 139, .10); }
        ...
        .assignment-card { padding: 0; overflow: hidden; border-color: #9cc9d8; }
        .assignment-hero { padding: 18px; background: linear-gradient(135deg, #16476a, #1f7fa3); border-bottom: 1px solid #b8d4e2; color: white; }
        .assignment-hero h2 { color: white; }
        .assignment-hero .muted { color: #dff3fa; }
        ...
        .assignment-stat { border: 1px solid rgba(255,255,255,.36); border-radius: 8px; background: rgba(255,255,255,.14); padding: 11px; }
        .assignment-stat span { display: block; color: #64748b; font-size: 12px; margin-bottom: 4px; }
        .assignment-stat span, .assignment-stat strong { color: white; }
        .assignment-stat strong { font-size: 22px; }
        .assignment-form { padding: 18px; }
        .assignment-select-card { border: 1px solid #b7d5e5; border-radius: 8px; padding: 12px; background: linear-gradient(135deg, #eef8fc, #ffffff); box-shadow: 0 8px 18px rgba(31, 127, 163, .08); }
        .assignment-select-card select { min-height: 54px; border-color: #5fa5c0; color: #16324f; font-weight: 800; background-color: white; }
        ...
        .assignments-head { display: flex; justify-content: space-between; gap: 12px; align-items: center; margin: -18px -18px 14px; padding: 16px 18px; background: linear-gradient(135deg, #e5f5fb, #ffffff); border-bottom: 1px solid #bfd6e4; }
        ...
        .assignment-table th { border-bottom: 0; padding: 0 10px 2px; color: #16476a; background: transparent; }
        .assignment-table td { background: linear-gradient(180deg, #ffffff, #eef8fc); border-top: 1px solid #bfd6e4; border-bottom: 1px solid #bfd6e4; padding: 13px 10px; }
        .assignment-table td:first-child { border-left: 5px solid #1f7fa3; border-radius: 8px 0 0 8px; }
        .assignment-table td:last-child { border-right: 1px solid #bfd6e4; border-radius: 0 8px 8px 0; }
        .assignment-table strong { color: #203a43; }
        .status-badge { display: inline-block; border-radius: 999px; padding: 5px 9px; background: #dff1f8; color: #16476a; font-weight: 800; font-size: 12px; border: 1px solid #b8d4e2; }
        ...
        .observation-head { padding: 18px; background: linear-gradient(135deg, #16476a, #1f7fa3); border-bottom: 1px solid #c5d8e2; color: white; }
        .observation-head h2 { color: white; }
        .observation-head .muted { color: #dff3fa; }
        ...
        .observation-context div { border: 1px solid rgba(255,255,255,.35); border-radius: 8px; background: rgba(255,255,255,.14); padding: 11px; }
        .observation-context span, .observation-context strong { color: white; }
        .observation-context span { display: block; font-size: 12px; margin-bottom: 4px; }
        .observation-context strong { font-size: 22px; }
        .observation-board { display: grid; gap: 14px; }
        .observation-toolbar { display: flex; justify-content: space-between; gap: 12px; align-items: center; margin: -18px -18px 0; padding: 18px; background: linear-gradient(135deg, #16476a, #1f7fa3); color: white; border-radius: 8px 8px 0 0; }
        .observation-toolbar h2 { margin-bottom: 4px; }
        .observation-toolbar p { margin: 0; line-height: 1.4; }
        .observation-toolbar h2, .observation-toolbar .muted { color: white; }
        .observation-count { border: 1px solid rgba(255,255,255,.38); border-radius: 8px; background: rgba(255,255,255,.14); padding: 10px 12px; color: white; font-weight: 800; white-space: nowrap; }
        .observation-list { display: grid; gap: 12px; }
        .observation-card { display: grid; grid-template-columns: 44px minmax(0, 1fr); gap: 12px; border: 1px solid #c4dce8; border-left: 5px solid #1f7fa3; border-radius: 8px; background: linear-gradient(180deg, #ffffff, #f2f9fd); padding: 14px; }
        ...
        .metric { border: 1px solid #b6d3e3; border-radius: 8px; background: linear-gradient(180deg, #ffffff, #e8f5fb); padding: 14px; box-shadow: 0 8px 18px rgba(31,111,139,.07); }
        ...
        .templates-panel { padding-top: 0; overflow: hidden; }
        .template-head { display: flex; justify-content: space-between; gap: 14px; align-items: flex-start; margin: 0 -18px 16px; padding: 18px; background: linear-gradient(135deg, #16476a, #1f7fa3); color: white; }
        .template-head h2, .template-head .muted { color: white; }
        .template-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(230px, 1fr)); gap: 16px; align-items: start; }
        .template-card { border: 1px solid #9cc9d8; border-radius: 8px; background: linear-gradient(180deg, #ffffff, #eef8fc); overflow: hidden; min-width: 0; box-shadow: 0 12px 24px rgba(31,111,139,.10); }
        .template-card:hover { border-color: #1f7fa3; box-shadow: 0 14px 28px rgba(31, 111, 139, .15); transform: translateY(-1px); }
        .template-preview { height: 150px; background: linear-gradient(135deg, #dff1f8, #f8fcff); display: grid; place-items: center; border-bottom: 1px solid #c5d8e2; overflow: hidden; }
        ...
        .template-body { padding: 14px; display: grid; gap: 12px; }
        ...
        .template-meta span { border: 1px solid #b9d8e6; border-radius: 7px; background: linear-gradient(180deg, #ffffff, #eaf6fb); padding: 8px 7px; color: #486174; font-size: 12px; line-height: 1.2; }
        ...
        @media (max-width: 900px) { .grid, .assignment-layout, .observations-layout, .result-filters { grid-template-columns: 1fr; } .checks { grid-template-columns: 1fr; } .assignments-head, .observation-toolbar { align-items: stretch; flex-direction: column; } .assignments-head .button { width: 100%; } }
...
<body>
    ...
    <main>
        ...
        <% } else if ("asignaciones".equals(seccion)) {
            GrupoEvaluacion grupoSeleccionado = grupoPorId(grupos, grupoAsignacionId);
            List<Evaluado> miembrosSeleccionados = grupoSeleccionado == null ? Collections.emptyList() : unicos(grupoSeleccionado.getEvaluados());
        %>
        <section class="assignment-layout">
            <div class="panel assignment-card">
                ...
                <form class="assignment-form" method="post">
                    ...
                    <label class="assignment-select-card">Prueba
                    ...
                    <label class="assignment-select-card">Grupo
            ...
            <div class="panel table-wrap">
                <div class="assignments-head">
                    <div>
                        <h2>Asignaciones por grupo</h2>
                        <p class="muted">Mostrando miembros de <strong><%= grupoSeleccionado == null ? "sin grupo" : h(grupoSeleccionado.getNombre()) %></strong>. Los intentos repetidos se revisan en Resultados.</p>
                    </div>
                </div>
                ...
                <article class="template-card">
                    ...
                    <div class="template-body">
                        ...
                        <div class="template-actions">
                            <a class="button primary" href="<%= request.getContextPath() %>/plantilla-wizard?id=<%= p.getId() %>">Editar</a>
        ...
        <section class="panel table-wrap">
            ...
            <div class="result-filters" data-result-filters="resultados-list">
                <label>Grupo
                    <select data-result-filter="grupo">
                        <option value="">Todos</option>
                        <% for (GrupoEvaluacion g : grupos) { %><option value="<%= g.getId() %>"><%= h(g.getNombre()) %></option><% } %>
                    </select>
                </label>
                <label>Prueba
                    <select data-result-filter="prueba">
                        <option value="">Todas</option>
                        <% for (Prueba p : pruebas) { %><option value="<%= p.getId() %>"><%= h(p.getNombre()) %></option><% } %>
                    </select>
                </label>
                <label>Evaluado
                    <select data-result-filter="evaluado">
                        <option value="">Todos</option>
                        <% for (Evaluado e : evaluados) { %><option value="<%= e.getId() %>"><%= nombre(e) %></option><% } %>
                    </select>
                </label>
                <label>Intento
                    <select data-result-filter="intento">
                        <option value="">Todos</option>
                        <option value="1">Intento 1</option>
                        <option value="2">Intento 2</option>
                        <option value="3">Intento 3</option>
                        <option value="4">Intento 4</option>
                        <option value="5">Intento 5+</option>
                    </select>
                </label>
            </div>
            <%
            ...
            <% for (AplicacionPrueba a : aplicaciones) { ResultadoPrueba r = a.getResultado(); int numeroIntento = intento(a, aplicaciones); %><tr data-filter-item="resultados-list" data-grupos="<%= idsGrupos(a.getEvaluado()) %>" data-prueba="<%= a.getPrueba() == null ? "" : a.getPrueba().getId() %>" data-evaluado="<%= a.getEvaluado() == null ? "" : a.getEvaluado().getId() %>" data-intento="<%= numeroIntento >= 5 ? 5 : numeroIntento %>" data-search="<%= nombre(a.getEvaluado()) %> <%= nombresGrupos(a.getEvaluado()) %> <%= h(a.getPrueba()) %> Intento <%= numeroIntento %> <%= h(a.getEstado()) %>"><td><%= nombre(a.getEvaluado()) %>
                <details>
    ...
    <script>
        const grupoAsignacion = '<%= "asignaciones".equals(seccion) && grupoAsignacionId != null ? grupoAsignacionId : "" %>';
        const selectorGrupoAsignacion = document.querySelector('.assignment-form select[name="grupoId"]');
        if (selectorGrupoAsignacion && grupoAsignacion) {
            selectorGrupoAsignacion.value = grupoAsignacion;
            selectorGrupoAsignacion.addEventListener('change', () => {
                if (selectorGrupoAsignacion.value) window.location.href = 'asignaciones?grupoId=' + encodeURIComponent(selectorGrupoAsignacion.value);
            });
        }
        document.querySelectorAll('[data-filter-input]').forEach(input => {
            ...
            const aplicar = () => {
                ...
                const filtros = document.querySelector('[data-result-filters="' + key + '"]');
                const grupo = filtros ? filtros.querySelector('[data-result-filter="grupo"]').value : '';
                const prueba = filtros ? filtros.querySelector('[data-result-filter="prueba"]').value : '';
                const evaluado = filtros ? filtros.querySelector('[data-result-filter="evaluado"]').value : '';
                const intento = filtros ? filtros.querySelector('[data-result-filter="intento"]').value : '';
                let visibles = 0;
                items.forEach(item => {
                    const matchText = !text || (item.dataset.search || item.textContent).toLowerCase().includes(text);
                    const matchGrupo = !grupo || (item.dataset.grupos || '').split(' ').includes(grupo);
                    const matchPrueba = !prueba || item.dataset.prueba === prueba;
                    const matchEvaluado = !evaluado || item.dataset.evaluado === evaluado;
                    const matchIntento = !intento || item.dataset.intento === intento;
                    const match = matchText && matchGrupo && matchPrueba && matchEvaluado && matchIntento;
            ...
            document.querySelectorAll('[data-result-filters="' + key + '"] select').forEach(select => select.addEventListener('change', aplicar));
        });
    </script>
</body>
</html>
